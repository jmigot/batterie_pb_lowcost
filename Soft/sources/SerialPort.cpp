...
const int SerialPort::TIMEOUT                 = 2; // Read/Write timeout for serial port (seconds)
const long SerialPort::QEXTSERIALPORT_TIMEOUT = 10; // QextSerialPort internal Read/Write timeout for serial port (milliseconds)
const int SerialPort::NB_ITERATIONS_CARD_FEED = 15; // Number of iterations for card feed (how many '?' to send to reset card buffers)
...
void SerialPort::connect()
{
	if ( ! this->isConnected())
	{
		...
		_window->setConnection("Connexion en cours...", Color::Blue);
		...
		while (i < ports.size() && ! this->isConnected())
		{
			...
			_port->setDtr(false);
			_port->setRts(false);
			_port->setTimeout(QEXTSERIALPORT_TIMEOUT);
			...
			if (_port->isOpen() && _port->isReadable() && _port->isWritable())
			{
				...
				while (j <= NB_ITERATIONS_CARD_FEED && ! this->isConnected())
				{
					_port->write("?", 1);
...
void SerialPort::feedCardBuffer()
{
	...
	{
		// Clear card buffers
		for (int i = 0; i < NB_ITERATIONS_CARD_FEED; i++)
		{
			// Write command
			this->write("?");
